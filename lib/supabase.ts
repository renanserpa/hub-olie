...
import {
    ...
    OrderNote,
    OrderPayment,
    // FIX: Added missing Analytics and Finance types to resolve "Cannot find name" errors.
    AnalyticsKPI,
    FinanceAccount,
    FinanceCategory,
    FinanceTransaction,
...
const handleError = (error: any, context: string) => {
    if (error?.code === '42P01') {
        console.warn(`[Supabase] Tabela ausente em "${context}".`);
...
export const supabaseService = {
  ...
  addManyDocuments: async <T extends { id?: string }>(table: string, docData: Omit<T, 'id'>[]): Promise<T[]> => {
  ...
  listenToDocument: <T extends { id: string }>(table: string, id: string, callback: (payload: T) => void) => {
      ...
      channel
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: table, filter: `id=eq.${id}` }, 
  ...
  // FIX: Added missing entity-specific methods used by hooks and other services.
  ...
  getMaterials: (): Promise<Material[]> => supabaseService.getCollection<Material>('config_materials', '*, config_supply_groups(name)'),
  getMaterialGroups: (): Promise<MaterialGroup[]> => supabaseService.getCollection<MaterialGroup>('config_supply_groups'),
  addMaterial: (data: any): Promise<Material> => supabaseService.addDocument<Material>('config_materials', data),
  addMaterialGroup: (data: any): Promise<MaterialGroup> => supabaseService.addDocument<MaterialGroup>('config_supply_groups', data),
  ...
  updateUser: (userId: string, data: Partial<UserProfile>): Promise<UserProfile> => supabaseService.updateDocument('profiles', userId, data),
  ...
  updateSetting: (category: SettingsCategory, id: string, data: any, subTab: string | null, subSubTab: string | null) => {
    ...
    const table = settingsTableMap[key] || subSubTab || subTab || category;
    return supabaseService.updateDocument(table, id, data);
  },
  ...
  deleteSetting: (category: SettingsCategory, id: string, subTab: string | null, subSubTab: string | null) => {
    ...
    if (subSubTab) key += `-${subSubTab}`;
    const table = settingsTableMap[key] || subSubTab || subTab || category;
    return supabaseService.deleteDocument(table, id);
  },
  ...
  updateWorkflowRule: (ruleId: string, isActive: boolean) => supabaseService.updateDocument<WorkflowRule>('workflow_rules', ruleId, { is_active: isActive }),
  ...
  revertSettingValue: async (historyId: string) => {
    const historyEntry = await supabaseService.getDocument<SystemSettingsHistory>('system_settings_history', historyId);
    ...
    await supabaseService.updateDocument<SystemSetting>('system_settings', currentSetting.id, { value: historyEntry.old_value });
  },
  ...
  addSetting: (category: SettingsCategory, data: any, subTab: string | null, subSubTab: string | null) => {
    ...
    if (subSubTab) key += `-${subSubTab}`;
    const table = settingsTableMap[key] || subSubTab || subTab || category;
    return supabaseService.addDocument(table, data);
  },
  ...
  getOrders: async (): Promise<Order[]> => {
    ...
    if (itemsError) return ordersData;
  ...
  getOrder: async (id: string): Promise<Order | null> => {
    const { data: orderData, error: orderError } = await supabase.from('orders').select('*, customers(*)').eq('id', id).maybeSingle();
  ...
  updateOrderStatus: async (orderId: string, newStatus: OrderStatus): Promise<Order> => supabaseService.updateDocument<Order>('orders', orderId, { status: newStatus, updated_at: new Date().toISOString() }),
  
  addOrder: async (orderData: Partial<Order>) => {
      const orderNumber = `OLIE-${new Date().getFullYear()}-${String(Date.now()).slice(-6)}`;
      const { items, ...orderFields } = orderData;
      const fullOrderData = { ...orderFields, number: orderNumber, created_at: new Date().toISOString(), updated_at: new Date().toISOString() };
      const savedOrder = await supabaseService.addDocument<Order>('orders', fullOrderData as any);
      if (items && items.length > 0) {
          const itemsWithOrderId = items.map(item => ({ ...item, order_id: savedOrder.id }));
          await supabaseService.addManyDocuments<OrderItem>('order_items', itemsWithOrderId as any);
      }
  },

  updateProductionOrderStatus: (id: string, status: ProductionOrderStatus) => supabaseService.updateDocument<ProductionOrder>('production_orders', id, { status, updated_at: new Date().toISOString() }),
  ...
  createPO: async (poData: { supplier_id: string; items: Omit<PurchaseOrderItem, 'id' | 'po_id'>[] }) => {
    ...
    const newPO = { supplier_id: poData.supplier_id, status: 'draft' as PurchaseOrderStatus, total, po_number: poNumber };
    const savedPO = await supabaseService.addDocument<PurchaseOrder>('purchase_orders', newPO as any);
    const itemsToSave = poData.items.map(item => ({...item, po_id: savedPO.id}));
    await supabaseService.addManyDocuments<PurchaseOrderItem>('purchase_order_items', itemsToSave as any);
  },
  ...
  receivePOItems: async (poId: string, receivedItems: { itemId: string; receivedQty: number }[]) => {
    ...
    for (const received of receivedItems) {
        ...
        if(!item) continue;
        await supabaseService.updateDocument<PurchaseOrderItem>('purchase_order_items', item.id, { received_quantity: item.received_quantity + received.receivedQty });
        await supabaseService.addInventoryMovement({ material_id: item.material_id, type: 'in', quantity: received.receivedQty, reason: 'RECEBIMENTO_PO', ref: po.po_number });
    }
    ...
    await supabaseService.updateDocument<PurchaseOrder>('purchase_orders', poId, { status: allReceived ? 'received' : 'partial' });
  },
  ...
  addInventoryMovement: (movementData: any) => supabaseService.addDocument('inventory_movements', movementData),
  
  transferStock: async (transferData: any) => {
    await supabaseService.addInventoryMovement({ material_id: transferData.material_id, type: 'out', quantity: -transferData.quantity, reason: 'TRANSFERENCIA_INTERNA', warehouse_id: transferData.from_warehouse_id, notes: `Para ${transferData.to_warehouse_id}` });
    await supabaseService.addInventoryMovement({ material_id: transferData.material_id, type: 'in', quantity: transferData.quantity, reason: 'TRANSFERENCIA_INTERNA', warehouse_id: transferData.to_warehouse_id, notes: `De ${transferData.from_warehouse_id}` });
  },
  ...
  updateSystemSetting: async (key: string, value: any) => {
    ...
    await supabaseService.updateDocument<SystemSetting>('system_settings', currentSetting.id, { value: newValueString });
  },
  ...
  testConnection: async () => {
    try {
        ...
        if (error) throw error;
        return { success: true, message: "Conexão com Supabase estável." };
    } catch (error: any) {
  ...
  testIntegrationConnection: async (id: string): Promise<void> => {
      ...
      await supabaseService.updateDocument<Integration>('config_integrations', id, {
          status: shouldFail ? 'error' : 'connected',
          last_sync: new Date().toISOString(),
          last_error: shouldFail ? 'Simulated connection timeout' : undefined,
      });
      await supabaseService.addDocument('integration_logs', {
          integration_id: id,
