...
// FIX: FieldConfig type is now imported from ../types.ts
...
const TabContent: React.FC<TabContentProps> = ({ category, data, fields, onAdd, onUpdate, onDelete, isAdmin, title }) => {
  ...
  // FIX: Refactored handleInputChange to be more type-safe and resolve the error on `e.target.checked`.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (currentItem) {
      ...
      // FIX: Cast formData to the expected type to resolve the assignment error.
  ...
  const handleDelete = async (itemId: string) => {
    if (window.confirm('Tem certeza que deseja excluir este item?')) {
        await onDelete(category, itemId);
    }
  }
  ...
  return (
    <div className="p-0">
      ...
      <div className="overflow-x-auto">
        ...
        {data.length === 0 && <p className="text-center text-textSecondary py-10">Nenhum item encontrado.</p>}
      </div>

      <Modal isOpen={isModalOpen} onClose={closeModal} title={currentItem ? `Editar Item` : `Adicionar Novo Item`}>
                ...
                <div key={field.key}>
                    ...
                    {field.type === 'checkbox' ? (
                        ...
                        <input
                            ...
                            onChange={handleInputChange}
                            className="w-full px-3 py-2 border border-input rounded-md shadow-sm bg-background focus:ring-ring focus:border-ring"
