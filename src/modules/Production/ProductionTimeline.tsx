...
const ProductionTimeline: React.FC<{ orders: (ProductionOrder & { tasks?: ProductionTask[] })[] }> = ({ orders }) => {
    ...
    const { timeRange, startDate } = React.useMemo(() => {
        ...
        // SAFEGUARD: If no valid dates exist across all tasks, use Due Dates or fallback to today
        if (validDates.length === 0) {
            ...
            // Default to a 30 day view starting from the earliest due date (or slightly before)
            return { timeRange: 30, startDate: new Date(minDate.getTime() - 7 * 86400000) };
        }

        const minDate = new Date(Math.min(...validDates.map(d => d.getTime())));
        const maxDate = new Date(Math.max(...validDates.map(d => d.getTime())));

        const range = differenceInDays(maxDate, minDate) + 5; // Add buffer
                    ...
                    {orders.map(order => (
                        <div key={order.id} className="p-2 border-b last:border-0">
                            <div className="flex justify-between items-center mb-1">
                                ...
                                <span className="text-xs text-textSecondary">Entrega: {new Date(order.due_date).toLocaleDateString('pt-BR')}</span>
                            </div>
                            <div className="relative h-8 mt-2 bg-secondary rounded w-full overflow-hidden">
                                {order.tasks && order.tasks.length > 0 ? (
                                    order.tasks.map(task => {
                                        const { left, width } = getTaskPosition(task);
                                        if (width === '0%' || parseFloat(width) <= 0) return null;
                                    ...
                                    <div className="text-xs text-center text-textSecondary pt-2 italic">Tarefas não agendadas</div>
                                )}
                            </div>
                        </div>
                    ))}
