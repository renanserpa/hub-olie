...
export const ProductionKanbanCard: React.FC<Props> = ({ order, onDragStart }) => {
  const plannedEnd = order.planned_end ? new Date(order.planned_end).toLocaleDateString('pt-BR') : null;
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <p className="text-xs uppercase tracking-wide text-slate-500">{order.reference}</p>
      ...
      <p className="mt-2 text-sm text-slate-700 dark:text-slate-200">
        Início: {new Date(order.planned_start).toLocaleDateString('pt-BR')}
      </p>
