...
import { Loader2, Mail, Lock, Database, Wifi } from 'lucide-react';
import { toast } from '../hooks/use-toast';
import { supabase } from '../lib/supabaseClient';
import BootstrapModal from './BootstrapModal';

const LoginPage: React.FC = () => {
  // Credenciais de resgate pré-preenchidas
  ...
  const handleTestConnection = async () => {
      ...
      try {
          ...
          if (error) {
              if (error.code === '42P01') {
                  toast({ title: 'Tabelas Ausentes', description: 'O banco de dados precisa ser configurado.', variant: 'destructive' });
                  setIsBootstrapOpen(true);
              } else {
                  toast({ title: 'Erro de Conexão', description: error.message, variant: 'destructive' });
              }
          } else {
              toast({ title: 'Conexão OK', description: 'Banco de dados acessível.', variant: 'default' });
          }
      } catch (e: any) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Pequeno delay para garantir que o cookie seja setado antes do reload
      setTimeout(() => window.location.href = '/', 500);
    } catch (err: any) {
      console.error(err);
      toast({ title: 'Falha no Login', description: err.message, variant: 'destructive' });
      // Se falhar por erro de tabela, sugere o bootstrap
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100 dark:bg-gray-900 p-4 font-sans">
      <div className="w-full max-w-md bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 border border-gray-200 dark:border-gray-700">
        <div className="text-center mb-8">
            ...
            <p className="text-sm text-gray-500">Acesso Administrativo (Recuperação)</p>
        </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input 
                    ...
                    className="w-full pl-9 pr-3 py-2 border rounded-lg focus:ring-2 focus:ring-yellow-500 outline-none"
                />
              </div>
            </div>
            <div>
        ...
        <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700 grid grid-cols-2 gap-2">
             <Button type="button" variant="outline" onClick={handleTestConnection} disabled={isLoading} className="text-xs">
                <Wifi className="w-3 h-3 mr-2" />
                Testar Conexão
             </Button>
             <Button type="button" variant="outline" onClick={() => setIsBootstrapOpen(true)} disabled={isLoading} className="text-xs">
